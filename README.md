...
4. **Controls** — only the match controller can pause/resume/abandon. Teams can self-service disable, e-stop, or leave mid-match. During countdown and autonomous, teams also get an **A-Stop** that stops their robot for the rest of auto and automatically re-enables it for teleop. Tapping Join also opens a small **match guide** window explaining what happens next; at the countdown it turns into a giant A-Stop button, and it closes itself when auto ends. Because it opens from the Join tap itself, no pop-up permission is needed — just keep it open until the match starts. If it got closed, allow pop-ups for the field site (or use the "Pop Out A-Stop" button) to bring it back.
