...
2. **Join & Ready** — teams choose an alliance (Red/Blue) from their station page. Joining hands the Driver Station to the field (disabled until the match starts — leave the match to drive freely). Ready unlocks once the Driver Station is actually talking to the FMS, so a match can't start against a dead link.
