...
2. **Join** — teams choose an alliance (Red/Blue) from their station page. Joining hands the Driver Station to the field (disabled until the match starts — leave the match to drive freely). If the Driver Station isn't talking to the FMS yet, the station page shows a warning, but teams can still ready up once the ready check opens.
3. **Ready check** — so teams can't sit "ready" indefinitely, nobody can ready up until everyone has joined and the controller **opens the ready check**. Only then do teams — and any required field staff (referee, scorekeeper, safety monitor, each from their own `/staff?role=…` page) — mark themselves ready. The controller can toggle any staff role to "not required" for a given match (a self-service field ignores them once and the choice sticks). Any roster change (a late join/leave/swap, or a config change) re-closes the check, so it always reflects the current lineup.
4. **Hold to Start** — once all joined teams and every required staff role are ready, the controller **holds** the start button through the whole 3-2-1 countdown. Letting go before the robots enable aborts the start — output is cut immediately and a fault sound plays; releasing once the start horn is playing does nothing. Phases then run automatically: countdown → auto → pause → teleop → endgame → post-match.
5. **Controls** — only the match controller can pause/resume/abandon. Teams can self-service disable, e-stop, or leave mid-match. E-stopping from the station page or match window always takes two taps to confirm. A stopped robot is recoverable mid-match: after an accidental disable (console button or the Enter key on the Driver Station) a **Re-enable** button appears on the station page and match window while robots are running; an e-stop needs field staff to clear it from the admin console first, after which the team (or staff) can re-enable. A disable applied by field staff can only be lifted by field staff. Teams also get an **A-Stop** that stops their robot for the rest of auto and automatically re-enables it for teleop — it can be armed any time from joining the match (e.g. when a team knows their auto won't run) through the end of auto, and cancelled up until the countdown starts. Tapping Join also opens a small **match window**: before the match it shows everyone's ready status with a Ready toggle, A-Stop pre-arm, and this robot's E-Stop (two taps to confirm); from the countdown it shows the phase and timer, dominated by a giant A-Stop button through auto, and it closes itself after the match. A checkbox lets it close at match start instead, and a 🔊/🔇 toggle plays the match audio (countdown, horn, phase cues) for that station — off by default, since the window is often next to the field speaker. Because it opens from the Join tap itself, no pop-up permission is needed — just keep it open. If it got closed, allow pop-ups for the field site (or use the "Pop Out" button) to bring it back.
6. **Post-match** — a short counting period runs after the buzzer (balls in flight). The match stays in post-match until the controller clears it, creates a new match, or 2 minutes pass — then it auto-clears back to free play (scoring included). Matches ended by e-stop never auto-clear; a human must clear them.
...
The admin page provides safety overrides: global e-stop, per-station e-stop/disable/enable, and force stop.

### Match Audio

Sound effects (charge horn, end buzzer, warning, pause/resume tones) play on phase transitions via a detected system audio player. A spoken "3… 2… 1…" announcer plays when the 3-second pre-start countdown begins: `countdown1.wav`–`countdown4.wav` are pre-timed clips (four different voices) with the numbers at 0/1/2 seconds and the charge horn baked in at exactly 3 seconds (no separate start sound plays after a countdown). The voice is picked per match by hashing the match id, so the field speaker and every open browser page agree. Each variant must remain a single clip: the server plays sounds through an exclusive ALSA device, so separate clips race each other and drop sounds. Place `.wav` files in `sounds/`.
...
| `VIDEO_PROXY_TARGET`          | _(none)_                | Base URL of a WHEP/WebRTC stream server (e.g. MediaMTX, `http://10.255.0.20:8889`). Enables `/api/video-proxy/*` so the scoreboard's video view can play `whep:<stream>` sources with sub-second latency over HTTPS. Only signaling is proxied; media flows directly over UDP. |
