...
4. **Controls** — only the match controller can pause/resume/abandon. Teams can self-service disable, e-stop, or leave mid-match. Teams also get an **A-Stop** that stops their robot for the rest of auto and automatically re-enables it for teleop — it can be armed any time from joining the match (e.g. when a team knows their auto won't run) through the end of auto, and cancelled up until the countdown starts. Tapping Join also opens a small **match guide** window explaining what happens next; at the countdown it turns into a giant A-Stop button, and it closes itself when auto ends. Because it opens from the Join tap itself, no pop-up permission is needed — just keep it open until the match starts. If it got closed, allow pop-ups for the field site (or use the "Pop Out A-Stop" button) to bring it back.
