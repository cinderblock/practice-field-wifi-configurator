...
4. **Controls** — only the match controller can pause/resume/abandon. Teams can self-service disable, e-stop, or leave mid-match. During countdown and autonomous, teams also get an **A-Stop** that stops their robot for the rest of auto and automatically re-enables it for teleop. The A-Stop is also available as a pop-out window (one giant button) that closes itself when auto ends — allow pop-ups for the field site on the driver station laptop and it opens automatically at match start; otherwise use the "Pop Out A-Stop" button.
