import type { RadioUpdate, StationName, StatusEntry } from '../src/types.js';
import { isValidRadioUpdate } from '../src/types.js';
...
class RadioManager {
  ...
  async configure(stationId: StationName, opts: null | { ssid: string; wpaKey: string }): Promise<void> {
    if (this.configuring) {
    ...
    if (!opts) {
      console.log('Clearing configuration for station:', stationId);

      delete this.activeConfig[stationId];
    } else {
      const { ssid, wpaKey } = opts;
    ...
    try {
      ...
      while (this.entries[this.entries.length - 1]?.radioUpdate.status !== 'CONNECTED') {
        await new Promise(resolve => setTimeout(resolve, 100));
      }
